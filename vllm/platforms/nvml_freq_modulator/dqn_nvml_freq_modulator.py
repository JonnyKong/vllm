...
class DQNNvmlFreqModulator(NvmlFreqModulator):
    ...
    def adjust(self) -> int:
        ...
        wait_queue_penalty = -1 if len(self.llm_engine.scheduler[0].waiting) > 5 else 0
        TBT_penalty = -10 if sys_stats['tbt_mean'] > self.TBT_SLO else 0
        reward = power_reward + wait_queue_penalty + TBT_penalty

        next_state = np.array([sys_stats['gpu_kv_cache_usage']], dtype=np.float32)
    ...
    def _load_model(self):
        if self.pretrained_rl_model_path is not None \
            ...
            self.policy_net.load_state_dict(torch.load(self.pretrained_rl_model_path, map_location=self.device))
            self.target_net.load_state_dict(self.policy_net.state_dict())
            print("Model loaded from", self.pretrained_rl_model_path)
        else:
            print("No pretrained model found at ", self.pretrained_rl_model_path)
    ...
    def __del__(self):
        torch.save(self.policy_net.state_dict(), Path(self.log_dir) / 'dqn_model.pth')
